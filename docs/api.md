...
Generated files include a stable marker comment and are skipped when already current. Each returned file includes `action: 'written' | 'unchanged'`.
...
- `clean` - Delete stale SchemaBridge-generated files that are no longer produced in the current run (default: `false`)
- `verify` - Compare generated files without writing or deleting; useful for CI (default: `false`)
- `trustedInput` - Required for loading trusted local schema files
...
`convertFolder` returns `written`, `unchanged`, `deleted`, and `outOfDate` counts, plus `warnings` and `errors`. In normal sync mode, changed files are written and `outOfDate` should be `0` after a successful run. In verify mode, files are not changed; `outOfDate > 0` means the generated output is stale.

Generated files include a stable marker comment, such as `# Generated by SchemaBridge. Do not edit.` for Python. `clean` only deletes stale files with this marker. It does not remove empty directories.

**CLI/API mapping:**

| CLI flag   | API option                 |
| ---------- | -------------------------- |
| `--init`   | `generateInitFiles: true`  |
| `--flat`   | `preserveStructure: false` |
| `--clean`  | `clean: true`              |
| `--verify` | `verify: true`             |
