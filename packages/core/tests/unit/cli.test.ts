...
describe('CLI runCLI', () => {
  ...
  describe('folder verify mode', () => {
    it('returns zero when generated files are already current', async () => {
      const tmp = await makeTempDir();
      const folderFixture = path.resolve('tests/fixtures/folder-structure');
    ...
    it('returns non-zero when generated files are out of date without rewriting them', async () => {
      const tmp = await makeTempDir();
      const folderFixture = path.resolve('tests/fixtures/folder-structure');
      const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
      ...
      const userPath = path.join(tmp, 'user_account_schema.py');
      ...
      expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('out of date'));
      await expect(fs.readFile(userPath, 'utf8')).resolves.toContain('BROKEN = True');
    });
  });

  describe('folder conversion errors', () => {
    it('returns non-zero when an exported schema fails to convert', async () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Failed to convert brokenSchema'),
      );
