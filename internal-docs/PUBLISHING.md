...
3. (Optional) Enter a `run_name` to make the manual run easy to identify in the Actions list.
4. Leave `publish_npm` / `publish_pypi` checked (default) to test both registries, or toggle either off if you only want to exercise one path.
5. The workflow will:
...
6. Verify the artifacts if desired:

   ```bash
   pip install --index-url https://test.pypi.org/simple schemabridge==0.1.1
   ```

7. Once satisfied, run the real release (GitHub Release or workflow dispatch with `dry_run=false`).
